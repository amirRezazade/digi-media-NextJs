...
export default function Pagination({ page, totalPage, onPage }) {
  return (
    <div className="flex justify-center items-center gap-1.5 md:gap-3 mt-12 text-sm p-2">
      <button disabled={page == 1} className={`flex justify-center items-center fill-black  disabled:fill-gray-500 dark:fill-white bg-stone-300 dark:bg-gray-700 cursor-pointer rounded w-10 h-8 ${page !== 1 ? "hover:bg-orange-400 hover:text-white" : ""} transition-colors duration-300`} onClick={() => onPage((prev) => Number(prev) - 1)}>
      ...
      <button disabled={page == totalPage} className={`flex justify-center items-center fill-black disabled:fill-gray-500 dark:fill-white bg-stone-300 dark:bg-gray-700  rounded w-10 h-8 ${page !== totalPage ? "hover:bg-orange-400 hover:text-white" : ""} transition-colors duration-300`} onClick={() => onPage((prev) => Number(prev) + 1)}>
