  ...
  return (
    <div className="flex flex-col bg-white dark:bg-gray-800 rounded-xl overflow-hidden">
      ...
      <div className=" px-2 py-2">
        ...
        <div dir="ltr" className=" w-full py-1.5 text-gray-500 dark:text-gray-300 text-xs overflow-hidden">
          <span dir="rtl" className="block w-full text-right">
            اثرات شناخته شده:
          </span>
          {actor.known_for?.slice(0, 3).map((item) => (
            <Link key={item.id} href={`/${item.media_type == "movie" ? "movie" : "series"}/${item.id}`} className="truncate block w-fit mt-1 hover:text-orange-400 ">
