...
export async function getActor(id) {
  let res = await fetch(`https://api.themoviedb.org/3/person/${id}?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}`, {
...
export async function generateMetadata({ params }) {
  ...
  let data = await getActor(actorsId);

  return {
    title: `digi-media | ${data ? data.name : ""}`,
...
export default async function page({ params }) {
  ...
  return (
    <main className=" px-3 md:px-6 lg:px-8 xl:px-12 my-8 md:my-7">
      <div className="flex items-center gap-1 text-[11px] dark:text-gray-300 mb-4 ">
        <span>
        ...
        <svg className="fill-gray-500 dark:fill-gray-300" width="12px" height="12px" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
          <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
          <g id="SVGRepo_iconCarrier">
            {" "}
            <path fillRule="evenodd" clipRule="evenodd" d="M15.7071 4.29289C16.0976 4.68342 16.0976 5.31658 15.7071 5.70711L9.41421 12L15.7071 18.2929C16.0976 18.6834 16.0976 19.3166 15.7071 19.7071C15.3166 20.0976 14.6834 20.0976 14.2929 19.7071L7.29289 12.7071C7.10536 12.5196 7 12.2652 7 12C7 11.7348 7.10536 11.4804 7.29289 11.2929L14.2929 4.29289C14.6834 3.90237 15.3166 3.90237 15.7071 4.29289Z"></path>{" "}
          </g>
        </svg>
        <Link href="/actors" className="transition-colors duration-300 cursor-pointer hover:text-orange-400">
          بازیگر ها
        </Link>
        <svg className="fill-gray-500 dark:fill-gray-300" width="12px" height="12px" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
        ...
        <span className="transition-colors duration-300 cursor-pointer hover:text-orange-400">{data.name}</span>
      </div>
