...
export default function MainData({ data }) {
  ...
  let [movies, setMovies] = useState(data?.results);
  ...
  useEffect(() => {
    ...
    router.push(`/movie?${params.toString()}`);
  }, [activeGenre, sortby]);
  ...
  const loadMore = async () => {
    try {
      setIsFetching(true);
      setRetry(false);
      const res = await fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&include_adult=false
  &vote_count.gte=100
  &vote_average.gte=1
  &popularity.gte=5
  ...
  return (
    <>
      <h1 className="font-bold text-2xl sm:text-3xl md:text-4xl xl:text-5xl mt-15 mb-13">{activeGenre === "all" ? "فیلم ها" : "فیلم های " + genres[activeGenre]}</h1>
      ...
      <div className=" flex items-center justify-between gap-2 my-5 ">
        <div className=" rounded-full px-1 lg:px-2 overflow-hidden backdrop-blur-lg border border-gray-400/50">
          <select onChange={(e) => setSortby(e.target.value)} className="text-xs sm:text-sm border-0 outline-0 cursor-pointer p-1 lg:p-2 ">
            ...
            <option className="bg-gray-900" value="release_date.desc">
              جدبدترین
            </option>
          </select>
        </div>
        <span className="text-sm py-1 px-3 rounded-full bg-transparent backdrop-blur-xl border border-gray-400/50">{data.total_results} فیلم </span>
      </div>
      {isLoading ? (
        ...
        <div className="mt-5 grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4  md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 gap-x-4 xl:gap-x-5 gap-y-6 sm:gap-y-8">
          {movies.map((item) => (
            <Cart key={item.id} type="movie" item={item} />
          ))}
        </div>
      ) : (
